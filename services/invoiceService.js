...
const { SwissQRBill } = require('swissqrbill/pdf'); // ← AJOUT pour QR Bill
...
class InvoiceService {
  ...
  // ============================================
  // MODIFIÉ : Génération de la page récapitulative avec QR Bill
  // ============================================
  static async generateTotalPage(doc, invoiceData) {
    ...
    const addInvoiceHeader = () => {
      ...
      doc.font('Helvetica-Bold').fontSize(14).text(`Invoice ${formattedOrderId}`, 50, titleY + 5);
      doc.font('Helvetica').fontSize(10).text(`Date: ${orderDate.toLocaleDateString('fr-CH')}`, 50, titleY + 25);
    ...
    const addSimpleTotalLine = (yPosition) => {
      ...
      // Espace pour le QR Bill (environ 105mm = 297 points)
      const qrBillHeight = 297;
      const availableHeight = pageHeight - qrBillHeight - yPosition - 20;
      const centerY = yPosition + (availableHeight / 2) - 30;
      
      const horizontalCenter = Math.floor(pageWidth / 2);
      
      doc.font('Helvetica-Bold').fontSize(14);
      doc.text(`TOTAL TTC: ${totalTTC.toFixed(2)} CHF`, horizontalCenter - 100, centerY, {
        width: 200,
        align: 'center'
      });
    ...
    addSimpleTotalLine(headerEndY);

    // ============================================
    // NOUVEAU : Génération du QR Bill suisse
    // ============================================
    try {
      const qrBillData = {
        currency: 'CHF',
        ...
        creditor: {
          ...
          zip: 1007,
          city: 'Lausanne',
          account: 'CH2380808009929375493', // ⚠️ REMPLACER PAR VOTRE IBAN RÉEL
          country: 'CH'
        },
      ...
      console.error('Erreur QR Bill:', error);
      
      // Fallback si erreur
      doc.font('Helvetica').fontSize(10);
      doc.text(
        'QR Bill error - Please contact us',
        50,
        doc.page.height - 100,
